...
PATH_FUNCTION="${PATH_BASE}/code/preprocessing/logfile/CreateBidsEventfile.R"
PATH_LOG="${PATH_BASE}/logs/preprocessing/logfile/CreateBidsEventfile/$(date '+%Y%m%d_%H%M')"
PATH_RETURN=$(pwd)
...
for SUB in ${SUB_LIST}; do
	...
	# Create job name job
	JOB_NAME="CreateBidsEventfile_${SUB}"
	...
	for SES in 1 2; do
		...
		echo Rscript ${PATH_FUNCTION} --sub_id ${SUB} \
		 ...
		 --save_timetable ${SAVE_TIMETABLE} >> job.slurm
